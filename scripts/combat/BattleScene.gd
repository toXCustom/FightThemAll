...
func _ready():
	...
	# Connect the button
	$UI/UpgradeButton.pressed.connect(_on_UpgradeButton_pressed)
...
func update_ui():
	...
	$UI/UpgradeButton.text = "Upgrade DPS\nCost: " + str(snapped(GameManager.get_upgrade_cost(), 0.1))
...
#
#Player auto-attacks  →  Monster loses HP  →  Monster dies
		#↓                                          ↓
  #Attack Timer fires                       Gold added to wallet
		#↑                                          ↓
  #Repeats forever                       Buy upgrades → more DPS
												   #↓
										#Next stage → harder monsters
